...
# mgs_cpm_log2 <- MRcounts(vgroups,norm=FALSE,log=TRUE,sl=1)
# mgs_cpm_log2_cds <- subset(mgs_cpm_log2, select=c(V4,V10,V13))
# mgs_cpm_log2_cs <- subset(mgs_cpm_log2, select=c(V5,V9,V11,V12))
# 
# mgs_cpm_log2_cds_medians <- rowMedians(mgs_cpm_log2_cds)
# mgs_cpm_log2_cs_medians <- rowMedians(mgs_cpm_log2_cs)
# 
# mgs_cpm_log2_cds_means <- rowMeans(mgs_cpm_log2_cds)
# mgs_cpm_log2_cs_means <- rowMeans(mgs_cpm_log2_cs)
# 
# mgs_cpm_log2_medians <- data.frame(cds=mgs_cpm_log2_cds_medians,cs=mgs_cpm_log2_cs_medians)
# mgs_cpm_log2_means <- data.frame(cds=mgs_cpm_log2_cds_means,cs=mgs_cpm_log2_cs_means)
# 
# ggplot(mgs_cpm_log2_means, aes(x = cds, y = cs)) + 
#   geom_point(size = 0.2) +
#   labs(x = "Viral Groups 27CDS Median CPM", y = "Viral Groups 55CS Median CPM") + 
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.text=element_text(size=4.0, color="black"), axis.title=element_text(size=5), axis.line.x = element_line(size = 0.3, linetype = "solid", colour = "black"), panel.border = element_blank(), axis.line.y = element_line(size = 0.3, linetype = "solid", colour = "black"), plot.title = element_text(size = 0.01), axis.ticks=element_line(size=0.3), legend.text=element_text(size=3.5), legend.key = element_blank(), legend.title = element_blank(), legend.key.size = unit(0.15, "cm"), legend.position = "none", legend.background = element_rect(fill=alpha('white', 0.0)))
```