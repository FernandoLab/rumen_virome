...
tiff("figure_2a.tiff", height = 6, width = 12, units='in', res=600)
...
tiff("figure_4a.tiff", width = 6, height = 6, units = 'in', res = 300)
...
```{r}
dm_list <- melt(total_bc_matrix)[melt(upper.tri(total_bc_matrix))$value, ]
total_map2 <- total_map[c("ID","Animal","Diet")]

names(total_map2)[1] <- "Var1"
dm_list2 <- merge(total_map2, dm_list, by="Var1")
names(dm_list2)[2] <- "var1_animal"
names(dm_list2)[3] <- "var1_diet"

names(total_map2)[1] <- "Var2"
dm_list2 <- merge(total_map2, dm_list2, by="Var2")
...
intra_diet$label <- "Intra-Diet"
intra_animal$label <- "Intra-Animal"
intra <- rbind(intra_diet, intra_animal)

tiff("figure_3a.tiff", width = 6, height = 6, units = 'in', res = 300)
ggplot(intra, aes(x = label, y = value)) + 
    ...
    theme(axis.text.x = element_text(colour = "black", size=19), axis.title.y = element_text(size = 19), 
