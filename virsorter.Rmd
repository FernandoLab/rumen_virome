...
python extract_virsorter_features.py --virsorter_csv=VIRSorter_global-phage-signal_1.5kb_viromes.csv --contig_fasta=vmg.illumina.contigs.1500.fasta --orf_fasta=vmg.illumina.orfs_aa.filter.fasta 
...
#use ublast instead. Using substription version so database fits into memory. Cant reproduce as a result.
...
#makeblastdb -in kegg_phast.fasta -dbtype prot -parse_seqids
#blastp -db kegg_phast.fasta -query vmg.illumina.orfs_aa.filter.virsorter.fasta -out vmg.illumina.orfs_aa.filter.virsorter.blastp_kegg.txt -evalue 0.00001 -outfmt "6 std qcovs qcovhsp" -max_target_seqs 1 -num_threads 10
```

Add python script to filter for AMGs.
...
Test if ORFs AMGs were identified on have differential abundance between 55CS and 27CDS. Try DESEQ2 first.
